...
def generate_explanation(context: str) -> str:
    ...
    prompt = f"""
                ...
                아래는 텍스트 데이터의 통계 요약입니다. 이 정보를 바탕으로 **객관적인 분석 요약**을 작성하세요.

                ✳️ 작성 지침:
                - 반드시 4~5개의 항목으로 구성하세요.
                - 각 항목 앞에는 번호(1., 2., 3., ...)를 붙이고 줄바꿈으로 구분하세요.
                - 각 항목은 **자연스럽고 서술형 문장**으로 작성하세요.
                - "→ 요약 시작:"이나 "→ 요약 끝:" 같은 표현은 절대 사용하지 마세요.
                - 단순 나열이 아니라 **통계 수치가 의미하는 바를 서술형으로 해석**하세요.
                - 예측이나 감성은 쓰지 말고, 데이터에 기반한 객관적 해석만 하세요.

                예시 형식:
                1. 총 문서 수는 6316개로, 대규모의 분석 가능한 텍스트 데이터셋입니다.
                2. 평균 문장 길이는 8단어로, 짧고 간결한 문장이 주를 이룹니다.
                3. 주요 키워드는 '여자', '사람', '남자', '말', '연기'로, 대화 중심의 데이터 특성을 보여줍니다.
                ...
