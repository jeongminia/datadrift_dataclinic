...
def custom_llm(model_name: str = "huihui_ai/exaone3.5-abliterated:7.8b",
    ...
    # 5. Ollama 모델 생성 (max_tokens, top_p 파라미터는 지원하지 않음)
    llm = Ollama(
        model=model_name,
...
def generate_explanation(dataset_name: str) -> str:
    ...
    # 5. generate_report.py에서 설정한 LLM 파라미터 가져오기
    llm_params = st.session_state.get('llm_generation_params')

    if not llm_params:
        st.warning("LLM 파라미터가 설정되지 않았습니다.")
        return "<p>LLM 파라미터가 설정되지 않았습니다.</p>"

    selected_model = llm_params.get('selected_model')
    temperature = llm_params.get('temperature')
    user_custom_prompt = llm_params.get('custom_prompt')

    # 6. 관련 지식 검색 - 사용자 설정 파라미터를 custom_llm에 전달
    knowledge_context = []
    vectorstore, llm = custom_llm(model_name=selected_model, model_temperature=temperature)
    ...
    if user_custom_prompt:
        ...
        custom_prompt = """다음 데이터셋의 드리프트 분석 결과를 전문가 관점에서 해석해주세요:
